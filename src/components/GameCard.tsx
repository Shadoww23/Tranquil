...
const platformStyles: Record<AnalyzedGame["platform"], string> = {
  ...
  Epic: "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300",
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <span className={`shrink-0 text-xs font-medium px-2 py-0.5 rounded-full ${platformStyles[game.platform]}`}>{game.platform}</span>
      </div>

      <p className="text-xs text-stone-400 dark:text-stone-500">
        <span className="font-semibold text-stone-600 dark:text-stone-300">{game.hoursPlayed.toLocaleString()}h</span> played
      </p>

      <div>
        <div className="flex items-center justify-between text-xs mb-1">
          <span className="text-stone-500 dark:text-stone-400">Design Risk Score</span>
          <span className="font-semibold text-stone-700 dark:text-stone-200">{designRiskScore.total}/100</span>
        </div>
        <div className="h-1.5 rounded-full bg-stone-100 dark:bg-stone-700 overflow-hidden" role="progressbar" aria-valuenow={designRiskScore.total} aria-valuemin={0} aria-valuemax={100} aria-label={`Design risk score: ${designRiskScore.total} out of 100`}>
          <div className={`h-full rounded-full ${riskBarColor(designRiskScore.total)}`} style={{ width: `${designRiskScore.total}%` }} />
        </div>
      </div>

      <div className="flex items-center gap-2">
        <span className="text-xs text-stone-500 dark:text-stone-400">Joy Index</span>
        <span className={`text-xs font-semibold ${joyColor(joyIndex.total)}`}>{joyIndex.total}/100 · {joyIndex.label}</span>
      </div>

      {detectedPatterns.length > 0 && (
        <div className="flex flex-wrap gap-1" aria-label="Detected design patterns">
          {visiblePatterns.map((p) => <PatternBadge key={p.pattern} pattern={p} />)}
          {hiddenCount > 0 && <span className="text-xs text-stone-400 dark:text-stone-500 self-center">+{hiddenCount} more</span>}
        </div>
      )}
