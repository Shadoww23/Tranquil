  ...
  return (
    <div className="rounded-2xl bg-white border border-stone-100 shadow-sm p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-semibold text-stone-800">Weekly Screen Time</h2>
        ...
        {weeklyScreenTime.map((d) => {
          ...
          return (
            <div key={d.day} className="flex flex-col items-center gap-1.5 flex-1">
              <span className="text-xs text-stone-400">{d.hours}h</span>
              <div className="w-full rounded-t-lg relative" style={{ height: "80px" }}>
                <div
                  className={`absolute bottom-0 w-full rounded-t-lg transition-all duration-700 ${
                    isToday ? "bg-emerald-400" : "bg-stone-200"
                  }`}
